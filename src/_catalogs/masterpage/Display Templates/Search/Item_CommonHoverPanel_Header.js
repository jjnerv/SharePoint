...
,'                    <img class="js-callout-closeButtonImage" src="', $urlHtmlEncodeString(GetThemedImageUrl('spcommon.png')) ,'" alt="', $htmlEncode(Srch.Res.hp_Tooltip_Close) ,'" />'
,'                </a>'
,'            </div>'
);
            if (showFileIcon) {
                var iconAltText = Srch.U.createFileIconAltText && Srch.U.createFileIconAltText(ctx.CurrentItem.IsContainer, ctx.CurrentItem.csr_OpenApp || ctx.CurrentItem.FileType) || "";
ms_outHtml.push(''
,'                <img class="ms-srch-item-icon ms-positionRelative" id="', $htmlEncode(id + Srch.U.Ids.icon) ,'" onload="this.style.display=\'inline\';this.style.bottom=\'-5px\'" src="', $urlHtmlEncodeString(Srch.U.getIconUrlByFileExtension(ctx.CurrentItem)) ,'" alt="', $urlHtmlEncodeString(iconAltText),'" />'
);
            }	
